...
export async function POST(request: Request) {
  try {
    ...
    const subscription = await razorpay.subscriptions.create({ plan_id: planId, total_count: 12, customer_notify: 1, notes: { organization_id: context.organization.id, plan: body.plan } })
    return NextResponse.json({ subscriptionId: subscription.id, keyId: process.env.RAZORPAY_KEY_ID, plan: selected.name })
  } catch {
